...
internal const val BANNED_TARGETS_PROPERTY_NAME = "abi.validation.kotlin.klib.targets.disabled.for.testing"
...
internal class KlibVerificationTests : BaseKotlinGradleTest() {
    ...
    fun `apiDump with native targets along with JVM target`() {
        ...
        runner.build().apply {
            ...
            val jvmApiDump = rootProjectDir.resolve("${_root_ide_package_.org.jetbrains.abi.tools.test.api.API_DIR}/testproject.api")
            assertTrue(jvmApiDump.exists(), "No API dump for JVM")

            val jvmExpected = _root_ide_package_.org.jetbrains.abi.tools.test.api.readFileList("/examples/classes/AnotherBuildConfig.dump")
